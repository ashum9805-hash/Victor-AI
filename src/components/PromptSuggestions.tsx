      ...
      {list.map((item, idx) => {
        ...
        return (
          <button
            ...
            className="flex flex-col text-left p-3.5 rounded-xl border border-zinc-200/80 bg-white/80 hover:bg-zinc-50 hover:border-zinc-300 active:scale-98 transition-all shadow-2xs group cursor-pointer"
          >
            <div className="flex items-center justify-between gap-2 mb-1.5">
              <span className="text-[10px] font-semibold tracking-wide uppercase px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-600">
                {item.tag}
              </span>
              <Icon className="w-3.5 h-3.5 text-zinc-400 group-hover:text-zinc-800 transition-colors" />
            </div>
            <span className="font-semibold text-xs text-zinc-900 mb-1">
              {item.title}
            </span>
